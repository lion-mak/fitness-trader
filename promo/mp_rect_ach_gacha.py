# -*- coding: utf-8 -*-
"""
mp_rect_ach_gacha.py —— 成就殿堂 / 涨停抽卡 两屏元素级对拍（⑥c）。

与 mp_rect_me.py 同机制（arm 9420 → 两端灌同一份存档 → 小程序一次会话量完 → playwright 量 PWA → 逐项相减）：
  · **两端先灌同一份存档**（promo/mock.json）—— 空存档下成就页只有「1 / 30 · 新兵」、
    抽卡页全是 ??? 锁卡，量不出「已达成 / MAX / 碎片进度」这些真正要看的东西；
    ⚠️ 不灌存档时两端状态不同，rect 差里混的是「内容不同」而不是「样式不同」。
  · 选择器一律加 **#page-xxx 作用域**（.backrow/.pagetitle/.card 被多个屏共用，
    裸 querySelector 会取到隐藏屏 ⇒ rect 全 0，看着像「PWA 缺失」）。
  · 每屏除 rect 外还取**文本**逐字比对 —— 文本不一致时，差值里就有「内容不同」的成分，
    必须先看那一栏再看 rect。
  · 底部留白口径 = 「`.page` 内最后一块**块级内容**的底边 ↔ 页底」，两端都该 = padding-bottom 80px。

本页特有的两类「两端不同源」配对（写进选择器表的 dict 形式）：
  1. `h2` / `p`：PWA 的 `.ach-hero h2` / `.ach-hero p` 是 **HTML 标签选择器**，
     小程序里映射成 wx-h2 / wx-p、静默失效 ⇒ 本页 .wxss 补了等价 class `.ah2` / `.ap`。
...
排除项（无同源选择器，另由运行时断言盯内容）：
  ...
  · 抽卡结果弹层（`.gacha-result`）默认 display:none，两端都 0×0 ⇒ 不测；
    弹层内容由 mp_gacha_test.js 的 D/E 段断言（10 张卡 / refund / 排序）。

用法：<venv>/python.exe promo/mp_rect_ach_gacha.py   （脚本自己 arm 9420）
输出：promo/_rect_ach_gacha_cmp.txt
"""
import io
import json
import os
...
GROUPS = [
    {
        "key": "achievements", "title": "成就殿堂",
        "route": "/pages/achievements/achievements", "tab": False,
        "switch": "showAchievements()",
        "sel": [
            ".backrow", ".backrow .arrow", ".backrow .tt",
            ".ach-hero", ".ach-hero .overlay",
            {"mp": ".ach-hero .ah2", "pwa": ".ach-hero h2"},
            {"mp": ".ach-hero .ap", "pwa": ".ach-hero p"},
            ".ach-cat", ".ach-cat-head", ".ach-cat-ic",
            ...
            ".prog-row", ".prog-row .ic", ".prog-row .meta", ".prog-row .meta .n",
            ".prog-row .meta .p", ".prog-row .bar", ".prog-row .bar .fill", ".prog-row .v",
        ],
        "probe": [".backrow .tt",
                  {"mp": ".ach-hero .ah2", "pwa": ".ach-hero h2"},
                  {"mp": ".ach-hero .ap", "pwa": ".ach-hero p"},
                  ".ach-cat-meta .n", ".ach-cat-prog", ".badge .lbl",
                  ".prog-row .meta .n", ".prog-row .v"],
        # 本屏最后一个顶层块 = 「近期目标」卡（`.card` 在本页共 1 张）
        "tailMp": ".card",
    },
    {
        "key": "gacha", "title": "涨停抽卡 · 图鉴",
        "route": "/pages/gacha/gacha", "tab": False,
        "switch": "showGacha()",
        "sel": [
            ".backrow", ".backrow .arrow", ".backrow .tt",
            ".pagetitle", ".pagetitle .k", ".pagetitle .t",
            ".gacha-hero", ".gacha-hero .g-balance-label", ".g-balance", ".g-collected",
            ".gacha-btns", ".gacha-btn.single", ".gacha-btn.ten",
            ".gacha-btn .big", ".gacha-btn .cost",
            ".card", ".card-title", ".card-title .t", ".card-title .m",
            ".rate-row", {"mp": ".rate-row .rt", "pwa": ".rate-row span"},
            ".gap-note",
            ".g-collect-track", ".g-collect-fill", ".g-collect-tip",
            ".ms-row", ".ms-info", ".ms-label", ".ms-pct", ".ms-btn",
            ".gacha-grid", ".g-cell", ".g-cell .g-emoji", ".g-name", ".g-rarity",
            ".g-star", ".g-prog", ".g-prog-fill", ".g-frag",
        ],
        "probe": [".backrow .tt", ".pagetitle .t", ".g-balance", ".g-collected",
                  ".gacha-btn .big", ".gacha-btn .cost", ".card-title .m",
                  ".ms-label", ".ms-pct", ".g-name", ".g-frag"],
        # 最后一块顶层块 = 「我的图鉴」卡
        "tailMp": ".card",
    },
]
...
PWA_META_JS = """([scope, padSel]) => {
    ...
    /* 「.page 内最后一块内容的底边」—— 直接取 #page-xxx 的最后一个子节点底边。
       为什么不用「max(被测选择器底边)」：那量的是「最后一个被测元素之下有多少空」，
       而两端在那之后的**内容本来就不同** ⇒ 量到的是内容差异，不是留白差异。 */
...
def main():
    p("=" * 104)
    p("成就殿堂 / 涨停抽卡 元素级对拍 —— 两端同为 CSS px，可直接相减")
    p("=" * 104)
    ...
    any_bad = 0
    try:
        # 2) 小程序侧两屏（必须同一会话量完：9420 会在每次会话结束时被 IDE 关掉）
        spec = []
        for g in GROUPS:
            mp_sels = []
        ...
        with sync_playwright() as pw:
            ...
            for g in GROUPS:
                ...
                p("%-30s %-24s %-24s %s" % ("选择器", "小程序 (l,t,w,h)", "PWA (l,t,w,h)", "差 (dl,dt,dw,dh)"))
                p("-" * 104)
                bad, miss = [], []
                for lab, a_s, b_s in pairs_of(g, "sel"):
                    ...
                    if a is None:
                        miss.append(lab)
                        p("%-30s %-24s %s" % (lab, "小程序缺失", "PWA 有"))
                        continue
                    if b is None:
                        miss.append(lab)
                        p("%-30s %-24s %s" % (lab, "有", "PWA 缺失"))
                        continue
                ...
                for lab, dd in bad:
                    p("     %-30s dl=%s dt=%s dw=%s dh=%s" % (lab, dd[0], dd[1], dd[2], dd[3]))
                if miss:
                    p("  未测到：%s" % ", ".join(miss))
                any_bad += len(bad)
                ...
                if tl.get("pageBottom") is None or pmeta.get("lastKidBottom") is None:
                    ...
                    p("  底部留白（.page 最后一块块级内容底边 ↔ 页底）：小程序 %.1f px / PWA %.1f px"
                      " ⇒ 差 %.1f px %s" % (mp_tail, pwa_tail, d, "✅" if abs(d) < 2 else "❌"))
    ...
    p("=" * 104)
    p("两屏合计差异超阈值：%d 个 %s" % (any_bad, "✅ 全部通过" if any_bad == 0 else "❌ 见上"))
